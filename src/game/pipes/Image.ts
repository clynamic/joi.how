...
export const imagePipe: Pipe = Composer.pipe(
  Composer.set<ImageContext>(['context', PLUGIN_NAMESPACE], {
    pushNextImage: image =>
      Events.dispatch({
  ...
  Events.handle(getEventKey(PLUGIN_NAMESPACE, 'pushNext'), event =>
  ...
  Events.handle(getEventKey(PLUGIN_NAMESPACE, 'setNextImages'), event =>
  ...
  Events.handle(getEventKey(PLUGIN_NAMESPACE, 'setImage'), event =>
    Composer.over<ImageState>(['state', PLUGIN_NAMESPACE], state => ({
      ...state,
